...
select_cell_type = True # Select partial cell types
selected_cell_type = ["CD14 Mono"] # Cell types for training should >=3.
for_discovery = True # If used for discovery, only positive samples are generated.
if_easy_neg = 1
neg_easy_ratio = 1

dataset_store_path = '../interaction_graph_datasets/PBMC_test'

rna_anndata = sc.read_h5ad("../original_datasets/10xMultiome-PBMC/10x-Multiome-Pbmc10k-RNA.h5ad")
print('rna_anndata:',rna_anndata) #9631 cells, 29095 genes
atac_anndata = sc.read_h5ad("../original_datasets/10xMultiome-PBMC/10x-Multiome-Pbmc10k-ATAC.h5ad")
...
sequences = []
max_len = 256
with open("../original_datasets/10xMultiome-PBMC/PBMC_peak_output.fa", 'r') as f:
    seq = ""
    for line in f:
        line = line.strip()
        if line.startswith(">"):
